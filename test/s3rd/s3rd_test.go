...
// 	"gitlab.12301.test/gopkg/idss-go-sdk/pb/s3rd"
...
// 	sid          = []int64{29742820, 6970, 3385}
// 	lid          = []int64{1}
// 	start        = types.Time(time.Now().AddDate(-1, 0, 0))
// 	end          = types.Time(time.Now())
// 	date         = types.Time(time.Now())
// 	compare      = types.Time(time.Now().AddDate(0, 0, -1))
// 	province     = "福建省"
// 	unknown      = true
// 	limit        = int64(10)
// 	codeCategory = []string{"A", "C", "M", "O", "G", "T", "R", "L"}
// )
...
// 	if len(grpcResp.CityList) == 0 {
// 		t.Fatalf("测试失败，CityList 数据返回为空")
// 	}

// 	if len(grpcResp.ProvinceList) == 0 {
// 		t.Fatalf("测试失败，ProvinceList 数据返回为空")
// 	}
